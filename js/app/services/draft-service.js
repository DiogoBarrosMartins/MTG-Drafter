define(function () {
        ...
        fetch(`https://api.scryfall.com/cards/search?q=set:${setCode}`)
          ...
          .then((cards) => {
            ...
            if (rareMythic.length > 0) {
              booster.push(
                mapCard(
                  rareMythic[Math.floor(Math.random() * rareMythic.length)]
                )
              );
            } else {
              booster.push(
                mapCard(commons[Math.floor(Math.random() * commons.length)])
              );
            }
            for (let i = 0; i < 3; i++) {
              booster.push(
                mapCard(uncommons[Math.floor(Math.random() * uncommons.length)])
              );
            }
            for (let i = 0; i < 11; i++) {
              booster.push(
  ...
  function passBoosterToNextPlayer() {
    ...
    for (var i = 0; i < players.length; i++) {
      ...
      console.log(
        nextPlayer.name + "got" + players[i].name + players[i].booster
      );
    }
  }
